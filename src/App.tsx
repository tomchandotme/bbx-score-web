  ...
  return (
    <div className="flex h-screen w-screen touch-manipulation overflow-hidden select-none">
      <div
        ...
        className="flex flex-1 cursor-pointer items-center justify-center bg-red-600 transition-colors active:bg-red-700"
      >
      ...
      {(currentScore.red >= MAX_SCORE || currentScore.blue >= MAX_SCORE) && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 animate-bounce rounded-full bg-yellow-400 px-6 py-2 text-xl font-bold text-black">
          {currentScore.red >= MAX_SCORE ? "RED WINS!" : "BLUE WINS!"}
        </div>
      )}
    </div>
  );
